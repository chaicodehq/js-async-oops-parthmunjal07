...
export class TempleQueue {
  ...
  static merge(queue1, queue2) {
    ...
    const newCapacity = queue1.length + queue2.length + 50;
    const mergedQueue = new TempleQueue(newName, newCapacity);
    const combinedDevotees = [...queue1.toArray(), ...queue2.toArray()];
    mergedQueue.#devotees = combinedDevotees;
  ...
  static fromArray(templeName, maxCapacity, arr) {
    // Your code here
    let newQueue = new TempleQueue(templeName, maxCapacity)

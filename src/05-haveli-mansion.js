...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    for (let i = 0; i < this.#residents.length; i++) {
      if (this.#residents[i].name === name) return  { success: false, message: "Already a resident!" }
    }
  ...
  removeResident(name, passcode) {
    // Your code here
    if (passcode !== this.#passcode) return { success: false, message: "Galat passcode!" }
    const resident = this.#residents.find((e) => e.name === name)
    if (!resident) return { success: false, message: "Resident nahi mila!" }
    let index = this.#residents.indexOf(resident)
  ...
  verifyAccess(name) {
    // Your code here
    const isAllowed = this.#residents.some((resident) => resident.name === name);
    const currentTime = new Date().toISOString();

    if (isAllowed) {
      this.#accessLog.push({ name: name, time: currentTime, allowed: true });
      return { allowed: true, message: `Swagat hai ${name}!` };
    } else {
      this.#accessLog.push({ name: name, time: currentTime, allowed: false });
  ...
  isResident(name) {
    // Your code here
    for (let i = 0; i < this.#residents.length; i++) {
      if (this.#residents[i].name === name) return true
    }
    return false
  }
}
